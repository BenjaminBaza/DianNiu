function read_ctrl_GrLFP()
    ...
    sol.time_tot    = 4000.0;                    %Total time of the simulation [s]  %4000.0
    sol.time        = 0.0; %10800;                     %Global variable keeping track of physical time during computation [s]
    ...
    if deb.prints>5
        ...
        csn_array=0:(p.csn_max)/50.:p.csn_max;
        cond_array = zeros(51,1);
        diff_array = zeros(51,1);
        Dsn_array = zeros(51,1);
        Dsp_array = zeros(51,1);
        ...
        subplot(3,5,[9,10])
        plot(c_array(2:51),diff_array(2:51),'LineWidth',2);
        title('electrolyte diffusivity','fontsize',fs);
        grid on
        grid minor

        subplot(3,5,[11,12])
        plot(csn_array,Dsn_array,'LineWidth',2);
        title('Neg electrode diffusivity','fontsize',fs);
        grid on
        grid minor

        subplot(3,5,[14,15])
        plot(c_array,Dsp_array,'LineWidth',2);
        title('Pos electrode diffusivity','fontsize',fs);
        grid on
        grid minor
