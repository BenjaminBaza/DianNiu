...
sol.time_tot    = 20.;%10800;                     %Total time of the simulation [s]
sol.dt          = 0.5;%1.                        %Time step for the time discretization [s]
...
p.Faraday = 96487 ;    	% Faraday constant, [Coulumbs/mol]
p.Rg = 8.3145 ;			% Perfect gas constant [j.mol/K]
p.Rfilm	= 0 ;			% Film layer ionic resistance [Ohm.m^2]


% Maximum concentrations
p.csn_max = 33133.0; % 3.6e3 * 372 * 1800 / p.Faraday;   % Max concentration in anode, [mol/m^3]
p.csp_max = 63104.0; % 3.6e3 * 247 * 5010 / p.Faraday;    % Max concentration in cathode, [mol/m^3]
...
ex.I_array  = 5.0 * ones(size(sol.time_array));%ex.temporary_Crate*ones(size(sol.time_array));  % Array containing the input current intensity at each time step (may be redundant)
...
ini.csp0 = 10;%16792.0;% 3.45e4;  % Initial pos. electrode concentration of Li, [mol/m^3]
ini.csn0 = 33000;%29866.1;%1.29e4;  % Initial neg. electrode concentration of Li, [mol/m^3]
ini.T0 = 298.15;%1e3;       % Initial temperature, [K]
ini.pe0 = 0.00001;      % Initial electrolyte electric potential, [V]
ini.psp0 = 0.00000;      % Initial positive electrode electric potential, [V]
ini.psn0 = 0.000;      % Initial negative electrode electric potential, [V]
...
if p.kappa_function_mode==1
	...
	fv.Ueq(sol.nb_cell_n+sol.nb_cell_s+1:sol.nb_cell) = param_functions.pos_electrode_Ueq(ini.csp0/p.csp_max,0)* ones(sol.nb_cell_p,1);
	if deb.prints>2
		disp(fv.Ueq)
	end
end
