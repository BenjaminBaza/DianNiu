classdef param_functions_LGM50_test
    ...
    methods
        ...
        function Ueqn = neg_electrode_Ueq(obj,c,i)
            ...
            x=c/p.csn_max;
            x=min(max(0,x),1);
            Ueqn = 0* x/(max(x,0.00000000001)); %1.9793 * exp(-39.3631 * x) + 0.2482 - 0.0909 * tanh(29.8538 * (x - 0.1234)) - 0.04478 * tanh(14.9159 * (x - 0.2769)) - 0.0205 * tanh(30.4444 * (x - 0.6103));
        end
        ...
        function Ueqp = pos_electrode_Ueq(obj,c,i)
            ...
            x=c/p.csp_max;
            x=min(max(0,x),1);
            Ueqp = 0* x/(max(x,0.00000000001)); % -0.8090 * x + 4.4875 - 0.0428 * tanh(18.5138 * (x - 0.5542)) - 17.7326 * tanh(15.7890 * (x - 0.3117)) + 17.5842 * tanh(15.9308 * (x - 0.3120));
        end
    end
end