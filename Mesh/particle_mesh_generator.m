function particle_mesh_generator(method)
	...
	if method==1
		...
		sol.part_coord_n  = zeros(1,sol.part_nb_cell+1);
		sol.part_coord_p  = zeros(1,sol.part_nb_cell+1);
		adjusted_position_a = zeros(1,sol.part_nb_cell+1);

		for i=1:1:sol.part_nb_cell+1
			proportion=(i-1)/sol.part_nb_cell;
			if i==1
				adjusted_position_a(i)=0;
			elseif i<=(sol.part_nb_cell+1)/2
				adjusted_position_a(i)=power(proportion,sol.particle_mesh_distribution_order) *0.5/power(0.5,sol.particle_mesh_distribution_order);
			else
				adjusted_position_a(i)=1-adjusted_position_a(sol.part_nb_cell + 2 - i);
			end
			sol.part_coord_n(i) = adjusted_position_a(i)*p.R_s_n;
			sol.part_coord_p(i) = adjusted_position_a(i)*p.R_s_p;
		end
